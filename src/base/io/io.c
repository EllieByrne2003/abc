...
int IoCommandRead( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "mcbgh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'm':
...
int IoCommandReadAiger( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "ch" ) ) != EOF )
    {
        switch ( c )
        {
            case 'c':
...
int IoCommandReadBlif( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "nmach" ) ) != EOF )
    {
        switch ( c )
        {
            case 'n':
...
int IoCommandReadCex( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "cxh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'c':
...
int IoCommandReadInit( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( argc != pOpt.optind && argc != pOpt.optind + 1 )
        goto usage;
...
int IoCommandReadPla( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "zbdxch" ) ) != EOF )
    {
        switch ( c )
        {
            case 'z':
...
int IoCommandReadPlaMo( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( argc != pOpt.optind + 1 )
        goto usage;
    // get the input file name
    pNtk = Mop_ManTest( argv[pOpt.optind], fMerge, fVerbose );
    if ( pNtk == NULL )
        return 1;
    // replace the current network
    Abc_FrameReplaceCurrentNetwork( pAbc, pNtk );
...
int IoCommandReadTruth( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "xfh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'x':
    ...
    if ( fFile )
    {
        FILE * pFile = fopen( argv[pOpt.optind], "rb" );
        if ( pFile == NULL )
        {
            ...
            fclose( pFile );
        pStr = Extra_FileReadContents( argv[pOpt.optind] );
    }
    else
        pStr = argv[pOpt.optind];
...
int IoCommandReadCnf( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "mh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'm':
    ...
    pFile = fopen( argv[pOpt.optind], "rb" );
    if ( pFile == NULL )
    {
        printf( "The file \"%s\" cannot be found.\n", argv[pOpt.optind] );
        return 1;
        ...
        fclose( pFile );
    vSops = Io_FileReadCnf( argv[pOpt.optind], fMulti );
    if ( Vec_PtrSize(vSops) == 0 )
    {
        Vec_PtrFreeFree( vSops );
...
int IoCommandReadVerilog( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "mcbh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'm':
...
int IoCommandWriteHie( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    if ( argc != pOpt.optind + 2 )
        goto usage;
    // get the output file name
    pBaseName = argv[pOpt.optind];
    pFileName = argv[pOpt.optind+1];
    // call the corresponding file writer
//    Io_Write( pAbc->pNtkCur, pFileName, Io_ReadFileType(pFileName) );
...
int IoCommandWriteAiger( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "scuvh" ) ) != EOF )
    {
        switch ( c )
        {
            case 's':
...
int IoCommandWriteBlif( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "Sjah" ) ) != EOF )
    {
        switch ( c )
        {
            case 'S':
...
int IoCommandWriteBench( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "lh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'l':
...
int IoCommandWriteCnf( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "nfpcvh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'n':
...
int IoCommandWriteCnf2( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "Kaiovh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'K':
                ...
                nLutSize = atoi(argv[pOpt.optind]);
                pOpt.optind++;
                break;
            case 'a':
                fNewAlgo ^= 1;
...
int IoCommandWriteCex( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "snmueocafzvhxt" ) ) != EOF )
    {
        switch ( c )
        {
            case 's':
...
int IoCommandWriteEdgelist( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "Nh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'N':
...
int IoCommandWriteHMetis( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "swvh" ) ) != EOF )
    {
        switch ( c )
        {
            case 's':
...
int IoCommandWritePla( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "Mmh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'M':
...
int IoCommandWriteVerilog( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "Kfamnh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'K':
...
int IoCommandWriteSortCnf( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "NQh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'N':
...
int IoCommandWriteTruth( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "xrh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'x':
...
int IoCommandWriteTruths( Abc_Frame_t * pAbc, int argc, char **argv )
{
    ...
    Extra_UtilOpt_t pOpt = { NULL, 0, NULL };
    while ( ( c = Extra_UtilGetopt( &pOpt, argc, argv, "rxbh" ) ) != EOF )
    {
        switch ( c )
        {
            case 'r':
