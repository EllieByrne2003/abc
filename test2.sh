...
for script in "${standard_scripts[@]}"; do
    run_and_append "Standard script: $script (with print_stats)" "$script; if -K 6; print_stats"
done
...
for script in "${iwls_scripts[@]}"; do
    run_and_append "IWLS script: $script (with print_stats)" "$script; if -K 6; print_stats"
done

# ----------------------------------------------------------------------
# Run lazy mans synthesis
# ----------------------------------------------------------------------
run_and_append "Lazy mans synthesis: recadd3 (with print_stats)" "rec_start3; recadd3; if -K 6; print_stats"

# ----------------------------------------------------------------------
# Run the user-provided custom script (as given)
# ----------------------------------------------------------------------
run_and_append "Custom script: $custom_cmds" "$custom_cmds; if -K 6; print_stats"
